...
interface PortfolioImage { id: string; url: string; alt: string }
interface Portfolio {
  id: string; slug: string; title: string; titleEn?: string; description: string
  location: string; year: number; category: string; status: string
  featured: boolean; coverImage: string; images: PortfolioImage[]
}

const categoryLabels: Record<string, string> = {
  'RESIDENTIAL': 'บ้านพักอาศัย', 'COMMERCIAL': 'พาณิชย์',
  'INTERIOR': 'ออกแบบภายใน', 'ARCHITECTURE': 'สถาปัตยกรรม'
}
const categories = ['ทั้งหมด', 'RESIDENTIAL', 'COMMERCIAL', 'INTERIOR', 'ARCHITECTURE']

export default function PortfolioClient({ projects }: { projects: Portfolio[] }) {
  const [activeCategory, setActiveCategory] = useState('ทั้งหมด')
  const [activeCard, setActiveCard] = useState<string | null>(null)

  const handleProjectClick = (e: React.MouseEvent, projectId: string) => {
    if (window.matchMedia('(hover: hover)').matches) return
    if (activeCard === projectId) { setActiveCard(null) }
    else { e.preventDefault(); setActiveCard(projectId) }
  }

  const filteredProjects = activeCategory === 'ทั้งหมด' ? projects : projects.filter(p => p.category === activeCategory)
          ...
          {categories.map((category) => (
            <button key={category} onClick={() => setActiveCategory(category)}
              className={`font-headline text-[0.6rem] sm:text-xs tracking-[0.15em] sm:tracking-[0.2em] font-bold px-4 sm:px-6 py-2 sm:py-3 rounded-lg transition-all ${
                activeCategory === category ? 'bg-primary text-white shadow-lg shadow-red-500/20' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}
            >
              ...
              {filteredProjects.map((project) => (
                ...
                >
                  <img alt={project.title}
                    className={`w-full object-cover transition-all duration-700 h-[220px] sm:h-[280px] lg:h-[320px] ${
                      activeCard === project.id ? 'grayscale-0 scale-105' : 'grayscale-0 sm:opacity-70 sm:grayscale-[40%] sm:group-hover:opacity-100 sm:group-hover:grayscale-0 sm:group-hover:scale-110'
                    }`}
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-t from-primary/80 via-primary/40 to-transparent transition-opacity duration-500 flex flex-col justify-end p-5 sm:p-6 md:p-8 ${
                    ...
                    <div className={`transform transition-transform duration-500 ${
                      ...
                      <p className={`text-white/90 text-xs sm:text-sm font-body ${activeCard === project.id ? 'block' : 'hidden sm:block'}`}>
                        {project.description}
                      </p>
